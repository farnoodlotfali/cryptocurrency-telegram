...
# abs ==> abstract
class SamanSadeghiChannel(AbsChannel):
    _channel_id = _config["CHANNEL_SAMAN_SADEGHI"]
    ...
    async def findPosition(self, msg)-> Optional[PositionSide]:
        position = None
        if "Buy".lower() in msg.lower():
            position = 'LONG'
        elif "sell".lower() in msg.lower():
            position = 'SHORT'
        
        try:
            position_value = await PositionSide.objects.aget(name=position.upper())
    ...
    async def findLeverageAndMarginMode(self, msg)-> tuple[Optional[MarginMode], Optional[int]]:
        leverage_match = re.search(r"lev\s*:\s*(\d+x)", msg, re.IGNORECASE)
        if leverage_match:
    ...
    def findStopLoss(self, msg)-> Optional[float]:
        sl = msg.replace(",","")
        stoploss = returnSearchValue(re.search(r"Sl\s*:\s*([\d.]+)", sl, re.IGNORECASE))
  
        return float(stoploss if stoploss else 'inf')

    # abs
    def findEntryTargets(self, msg)-> Optional[list[float]]:
        match = re.search(r"Entry\s*:\s*([\d.,]+)", msg, re.DOTALL | re.IGNORECASE)
        if match:
    ...
    def findTakeProfits(self, msg)-> Optional[list[float]]:
        match = re.search(r"Tp\s*:\s*([\d._]+)", msg, re.DOTALL | re.IGNORECASE)
        if match:
            entry_str = match.group(1) 
            return [float(x.strip()) for x in entry_str.split('_')]
