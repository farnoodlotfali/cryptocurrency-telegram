...
# abs ==> abstract
class KingCryptoChannel(AbsChannel):
    _channel_id = _config["CHANNEL_KING_CRYPTO"]
    ...
    async def findLeverageAndMarginMode(self, msg)-> tuple[Optional[MarginMode], Optional[int]]:
        leverage_match =re.search(r"LV\s*:\s*([\d._]+)", msg, re.IGNORECASE)
        if leverage_match:
    ...
    def findStopLoss(self, msg)-> Optional[float]:
        sl = msg.replace(",","")
        stoploss = returnSearchValue(re.search(r"Stop\s*:\s*([\d.]+)", sl, re.IGNORECASE))
  
        return float(stoploss if stoploss else 'inf')

    # abs
    def findEntryTargets(self, msg)-> Optional[list[float]]:
        match = re.findall(r"Entry\d+: ([\w\.]+)", msg, re.DOTALL | re.IGNORECASE)
        if match:
            for x in match:
                if x.strip().lower() == OrderType.MARKET.value:
                    match.remove(x)
                    match.insert(0, OrderType.MARKET.value)


            return [float(x.strip()) if x != OrderType.MARKET.value else x for x in match]
    
    # abs
    def findTakeProfits(self, msg)-> Optional[list[float]]:
        targets = re.findall(r"Target \d+ : ([\d\.]+)", msg, re.DOTALL | re.IGNORECASE)
        if not targets:
            targets = re.findall(r"Target :([\d\._]+)", msg, re.DOTALL | re.IGNORECASE)
            if targets:
                targets = targets[0].split('_')
        return [float(x.strip()) for x in targets]
