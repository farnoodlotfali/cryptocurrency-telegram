...
class Strategy14(AbsStrategy):
    strategy_name = 'strategy14'

    async def backtest_with_money_strategy_14(self, predicts: list[Predict], showPrint: bool= False, max_percent_stoploss: float= 2, max_percent_tp: float= 4, positionSize: float= 100):
        ...
        for i, pr in enumerate(predicts):
            try:
                ...
                positionName = pr.position.name
                isShort = positionName == PositionSideValues.SHORT.value

                # tp = stoploss.value
                tp = calStoploss(entry=new_entry,isShort=not isShort,leverage=leverage,max_percent_stoploss=max_percent_tp)

                stop_loss = calStoploss(entry=new_entry,isShort= isShort,leverage=leverage,max_percent_stoploss=max_percent_stoploss )
                ...
                is_hit = False
                # print(f'tp: {tp}, new_entry: {new_entry}, stop_loss: {stop_loss}, positionName: {positionName}, leverage: {leverage}')
                # tohlcv
                # row[0] = timestamp
                # row[1] = open
                # row[2] = high
