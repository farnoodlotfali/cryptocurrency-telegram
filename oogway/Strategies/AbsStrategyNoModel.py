...
class AbsStrategyNoModel(ABC):
    ...
    def addPendingOrder(self, order:PredictNoModel):
    ...
    def removeFromPending(self, order:PredictNoModel):
    ...
    def addMissOrder(self, order:PredictNoModel):
    ...
    def addEntryOrder(self, order:PredictNoModel, active_date:int):
        """
    ...
    def orderDetailController(self, predict: PredictNoModel, stopLoss:float, take_profit_targets: list, entry_targets: list, stopLoss_time:list):
       
        model = {}
        model['symbol'] = predict['symbol']
        model['market'] = predict['market']
        model['position'] = predict['position']
    ...
    def addActiveOrder(self, order:PredictNoModel, positionSize:float):
    ...
    def report(self, name:str='sr10'):
        ...
        df.to_csv(f'{path_folder}/report-NoModel-{date}-{hour}-{minute}-{second}-{self.strategy_name}.csv', index=False)
