...
class Strategy12(AbsStrategy):
    strategy_name = 'strategy12'

    async def backtest_with_money_strategy_12(self, predicts: list[Predict], close_tp:int=1, showPrint: bool= False, positionSize: float= 100, trendPeriod:int= 50, risk_percent:float= 1):
        
        if not (0.05 <= risk_percent <= 1):
            raise ValueError(f"risk_percent should be between 0.05 and 1, but got {risk_percent}")

        initial_positionSize = positionSize
        ...
        for i, pr in enumerate(predicts):
            try:
                ...
                positionSize = initial_positionSize
                ...
                if (isSHORT and trend == TrendValues.UPTREND.value) or (not isSHORT and trend == TrendValues.DOWNTREND.value):
                    ...
                    if accept:
                        # cal positionSize according to risk percent
